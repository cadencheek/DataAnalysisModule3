...
where categories.name = "Beans" and (select max(price) from products); 
...
where (select category_name from categories join products on categories.product_id = products.product_id where category_name = 'Merch')
